...
class Panair(ExternalCode):
    ...
    def __init__(self, na, network_info, sym_plane_index=None):
        super(Panair, self).__init__()

        #Number of aerodynamic grid points (excluding the root points)
        self.na = na
        ...
        if sys.platform == 'win32':
            self.options['command'] = ['cmd.exe', '/c', r'panair.exe', self.input_filepath]
        else:
    ...
    def solve_nonlinear(self, params, unknowns, resids):
        ...
        super(Panair, self).solve_nonlinear(params, unknowns, resids)
